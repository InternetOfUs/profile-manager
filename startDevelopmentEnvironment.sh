...
docker run --name wenet_profile_manager_dev -v /var/run/docker.sock:/var/run/docker.sock -v ${HOME}/.m2/repository:/root/.m2/repository  -v ${PWD}:/app -it wenet/profile-manager:dev /bin/bash
popd >/dev/null